...
class TextDS:
    def __init__(self, path, preprocessor=Preprocessor(), num_terms=10000, batch_size=__settings__.batch_size, split=__settings__.split, window_size=__settings__.window_size, dev='cpu'):
        ...
        if(__settings__.split == (0.95, 0.025)):
        ...
        self.split(__settings__.split[0], __settings__.split[1])
    ...
    def batch_method(self, batch):
        ...
        for sentence in sentences:
            if (len(sentence) < w * 2 + 1):
                continue
            full_df_sentence = np.array([sentence[i - w:i + w + 1] for i in range(w, len(sentence) - w)])
            if (len(full_df) != 0):
                full_df = np.r_[full_df, full_df_sentence]
            else:
                full_df = full_df_sentence
        x = np.delete(full_df, w, axis=1)
        y = full_df[:, w]
...
class LabelledTextDS(TextDS):
    ...
    def get_vector_representation(self):
        ...
        vectorizer = CountVectorizer(lowercase=False,
                                     ...
                                     stop_words=None,
                                     max_features=25000,  ## This only keeps the 100 most frequent words as features
                                     ngram_range=(1, 2),  ## This uses only unigram counts
                                     binary=True)  ## This sets the beatures to be frequency counts
